...
def start_playing():
    ...
    min_wait_time = int(min_wait_time_entry.get())
    max_wait_time = int(max_wait_time_entry.get())
    status_label.config(text="Program Çalışıyor")
    play_thread = Thread(target=continuous_play, args=(folder_path, play_time, min_wait_time, max_wait_time))
    play_thread.start()

def continuous_play(folder_path, play_time, min_wait_time, max_wait_time):
    while True:
...
# Min. bekleme süresi metin kutusu
min_wait_time_label = tk.Label(window, text="Min. Bekleme Süresi (saniye):")
min_wait_time_label.pack(pady=10)
min_wait_time_entry = tk.Entry(window)
min_wait_time_entry.pack(pady=5)

# Max. bekleme süresi metin kutusu
max_wait_time_label = tk.Label(window, text="Max. Bekleme Süresi (saniye):")
max_wait_time_label.pack(pady=10)
max_wait_time_entry = tk.Entry(window)
max_wait_time_entry.pack(pady=5)
